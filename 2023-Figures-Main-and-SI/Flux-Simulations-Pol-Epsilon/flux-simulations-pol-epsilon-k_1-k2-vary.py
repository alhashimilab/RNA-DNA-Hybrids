...
def dxdt(x, T, *args):
    ...
    A, B, C, D, E = x
    k1, k_1, k2, k_2, k3, k_3, k4, k_4, k5, k_5 = args

    dAdt = k5*E - k_5*A -k1*A + k_1*B - k2*A + k_2*C
...
def sim(params=(5.74, 6127.67, 10.82, 3642, 2437, 769, 268, 100, 100, 70000)):
    ...
    k1, k_1, k2, k_2, k3, k_3, k4, k_4, k5, k_5 = params
    C0 = np.array([0, 0, 0, 0, 1])
    Data = odeint(dxdt, C0, time, args = params)[-1]
    GS, ES1, ES2, Mis, G = Data
    # FTvT = (1/(k1*GS))**-1
    # FTvA = ((1/(GS*k2) + (1/(ES2*k_3))))**-1
    # FAvA = (1/(k2*GS))**-1
    # FAvT = ((1/(GS*k1)) + (1/(ES1*k3)))**-1
    
    Ftt = ((1/(G*k5)) + (1/(GS*k1)) + (1/(ES1*k4)))**-1
    Fta = ((1/(G*k5)) + (1/(GS*k2)) + (1/(ES2*k_3)) + (1/(ES1*k4)))**-1
    Faa = ((1/(G*k5)) + (1/(GS*k2)) + (1/(ES2*k4)))**-1
    Fat = ((1/(G*k5)) + (1/(GS*k1)) + (1/(ES1*k3)) + (1/(ES2*k4)))**-1
    FT = Ftt + Fta
    FA = Faa + Fat 
    fT = FT / (FT + FA)
    fA = FA / (FT + FA)

    return [G, GS, ES1, ES2, Mis, FT, FA, fT, fA]
...
if __name__ == "__main__":
    ...
    params_file = 'params_for_python_flux/params_for_flux_calc_CGC_RNA.csv'
    path = os.path.join(parent_path,params_file)
    params_table = read_table(path)

    # create variable k2 and k_1
    k2s = create_vector(115, 0.001, 1.235)
    k_1s = create_vector(130, 25., 1.125)

    out_folder = 'Flux-Simulations-Pol-Epsilon/k2s_k_1s_all/sims_CGC_RNA/sims_CGC_RNA'
    out_path = os.path.join(parent_path,out_folder)

    for k_1 in k_1s:
        out_file = out_path + '_' + '{0:.1f}'.format(k_1) + '.csv'
        with open(out_file, 'w') as out:
            l = 'G,GS,ES1,ES2,Mis,FT,FA,fT,fA\n'
            out.writelines(l)
            for params in params_table:
                for k2 in k2s:
                    comp_params = np.concatenate((params,np.array([k2,100,100,k_1])))
